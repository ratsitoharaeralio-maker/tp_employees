...
<body>
    <div class="container">
        ...
        <?php } else { ?>
            <h1><?= $employee['first_name'] ?>     <?= $employee['last_name'] ?> — devenir manager de
                <?= $current_dept['dept_name'] ?>
            </h1>

            <?php if ($success) { ?>
                <p class="alert alert-success">C'est fait : l'employé est désormais le manager du département.
            ...
            <div class="card">
                <form method="post" action="become_manager.php?emp_no=<?= urlencode($emp_no) ?>">
                    ...
                    <p><input type="submit" class="btn" value="Devenir manager"></p>
                </form>
            </div>
        <?php } ?>
    </div>
